...
path = r"C:\Usoject\master\masterLTAG\GISUL\finale\ULpointeau.gpkg"
...
#     d=PointEau(
#     nom= row['nom'] ,secteur=row['secteur'],source=row['source'],
#     fonctionnel=row['fonctionnel'],agent_collecteur=row['agent_collecteur'],limite=LimiteGeographique.objects.get(id=row['limite_id']),zone=Zone.objects.get(id=row['zone_id']),
#     geometrie=GEOSGeometry(row['geometry'].wkt,srid=32631)
#     )
#     d.save()
#PointEau.objects.all().delete()
#fonctionnel=['fonctionnel'],longitude=['longitude'],latitude=['latitude'],route_id=['route_id'],zone_id=['zone_id'],limite_id=['limite_id'],patking_id=['patking_id'],